...
class FreshmanViewController: UIViewController, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let postString = "type=1&o=0&user="+usernameSet+"&group=0";
    ...
    func tableView(_ postTableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.onLikeButtonTapped = {
            ...
            let myUrl = URL(string: "http://www.bruincave.com/m/ios/likepostaction.php");
            
            var request = URLRequest(url:myUrl!)
            
            request.httpMethod = "POST"// Compose a query string
            
            let postString = "username="+username+"&postid="+String(self.idArray[indexPath.row])
                            ...
                            for post in postArray{
                                if let postDict = post as? NSDictionary {
                                    if let login = postDict.value(forKey: "loginError") {
                                        loginError = login as! Bool
                                        
                                        if(self.likedByMeArray[indexPath.row] == 0){
                                            if let image = UIImage(named: "favoritefilled.png") {
                                                cell.likedByMeImage.setImage(image, for: .normal)
                                            }
                                        }else{
    ...
    func loadMore() {
        ...
        offset = offset + 5
        let postString = "type=1&o="+String(offset)+"&user="+usernameSet+"&group=0"
